...
import { LogLevel, SinkFunction } from "./types.ts";
...
/** Create a new Logger */
export function createLogger({ minimumLevel = LogLevel.DEBUG, outputFormat = textFormat }: Partial<LoggerOptions> = {}): Logger {
...
class LoggerImpl implements Logger {
	...
	addSink(sinkFunc: SinkFunction, outputFormat: string = this.outputFormat): this {
		this.sinks.push({
			sinkFunc,
			outputFormat,
		});
	...
	private log(level: LogLevel, format: string, ...args: unknown[]): void {
		...
		for (let sink of this.sinks) {
			const parsed = parseLogEntry(level, format, sink.outputFormat, ...args);
